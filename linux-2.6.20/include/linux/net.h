...
#define NPROTO		32		/* should be enough for now..	*/	// 协议族数量
...
#define SOCK_ASYNC_NOSPACE	0			// 标识套接口发送队列是否满
#define SOCK_ASYNC_WAITDATA	1			// 标识应用程序通过recv调用时，是否在等待数据的接收
#define SOCK_NOSPACE		2			// 标识在非异步的情况下，该套接口的发送队列是否满
#define SOCK_PASSCRED		3			// 标识是否设置了SO_PASSCRE套接口选项
#define SOCK_PASSSEC		4			// 标识是否设置了SO_PASSSEC套接口选项
...
// 套接口层数据结构
struct socket {
	socket_state		state;			// 套接口所处的状态标志。有一些状态只对TCP有效
	unsigned long		flags;			// 标志位 例如: SOCK_ASYNC_NOSPACE
	/*
		传输层协议相关的套接口层操作集合.
		TCP: inet_stream_ops
		UDP: inet_dgram_ops
	...
	*/
	...
	/*
		sk指向传输层。
		sk根据协议不同，分别指向TCP、UDP、RAW层，分别对应tcp_sock udp_sock raw_sock
...
// 协议族信息结构 PF_INET inet_family_ops
struct net_proto_family {
	int		family;				// 协议族对应的协议族常量 PF_INET
	int		(*create)(struct socket *sock, int protocol);	// 套接口创建函数指针
	struct module	*owner;
};
