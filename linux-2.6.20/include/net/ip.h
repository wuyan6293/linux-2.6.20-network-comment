...
// 描述IP控制信息块，存储有关输出的控制信息
struct ipcm_cookie
{
	__be32			addr;			// UDP、RAW数据报的目的地址。
	int			oif;				// UDP、RAW数据报的输出网络设备
	struct ip_options	*opt;		// 如果不为NULL，则指向发送数据报的IP选项信息
};
...
struct ip_reply_arg {
	struct kvec iov[1];   		// 待发送数据
	__wsum 	    csum;			// TCP伪首部校验和
	int	    csumoffset; /* u16 offset of csum in iov[0].iov_base */
				/* -1 if not needed */ 	// 校验和在首部中的偏移
}; 
