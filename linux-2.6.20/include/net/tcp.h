...
/* urg_data states */
#define TCP_URG_VALID	0x0100			// 标识紧急数据是有效的，用户可以读取
#define TCP_URG_NOTYET	0x0200			// 标识接收到的段中存在紧急数据
...
/* Flags in tp->nonagle */
#define TCP_NAGLE_OFF		1	/* Nagle's algo is disabled */						// 关闭Nagle算法
#define TCP_NAGLE_CORK		2	/* Socket is corked	    */							// 对Nagle算法进行优化，使发送的段尽可能携带更多的数据，但是有个200ms时间限制，一旦超时会被排入发送队列进行发送
#define TCP_NAGLE_PUSH		4	/* Cork is overridden for already queued data */	// 正常的nagle算法，不对nagle算法进行优化
...
#endif
	} header;	/* For incoming frames		*/					// IPv4 IPv6的头部
	__u32		seq;		/* Starting sequence number	*/		// 当前段开始序号
	__u32		end_seq;	/* SEQ + FIN + SYN + datalen	*/	// 当前段开始序号加上当前段数据长度，如果标志域中存在SYN或FIN标志，则还需要加1
	__u32		when;		/* used to compute rtt's	*/		// 段发送时间及段发送时记录的当前jiffies值
	__u8		flags;		/* TCP header flags.		*/		// 记录原始TCP首部标志。
...
#define TCPCB_FLAG_CWR		0x80
	...
	__u8		sacked;		/* State flags for SACK/FACK.	*/	// 主要用来描述段的重传状态，同时标识包是否包含紧急数据。
...
#define TCPCB_AT_TAIL		(TCPCB_URG)
	...
	__u16		urg_ptr;	/* Valid w/URG flags is set.	*/	// 如果存在TCPCB_FLAG_URG标志，则说明TCP段中有紧急数据，而urg_ptr则用来保存TCP首部中紧急指针值
	__u32		ack_seq;	/* Sequence number ACK'd	*/		// 接收到的TCP段首部中的确认序号
};
