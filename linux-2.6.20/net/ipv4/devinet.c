...
struct in_device *inetdev_init(struct net_device *dev)
{
	...
	if (!in_dev)
		goto out;
	INIT_RCU_HEAD(&in_dev->rcu_head);				// 初始化RCU
	memcpy(&in_dev->cnf, &ipv4_devconf_dflt, sizeof(in_dev->cnf));		// 初始化IP配置块
	in_dev->cnf.sysctl = NULL;
	in_dev->dev = dev;	  			// 初始化所属成员
	if ((in_dev->arp_parms = neigh_parms_alloc(dev, &arp_tbl)) == NULL)// 初始化ARP信息结构
		goto out_kfree;
	/* Reference in_dev->dev */
	dev_hold(dev);			// 增加net_device引用计数
#ifdef CONFIG_SYSCTL
	...
	in_dev_hold(in_dev);	// 增加in_device引用计数		

#ifdef CONFIG_SYSCTL
	devinet_sysctl_register(in_dev, &in_dev->cnf);	// 为IP注册相关的系统参数
#endif
	ip_mc_init_dev(in_dev);				// 初始化IGMP模块
	if (dev->flags & IFF_UP)
		ip_mc_up(in_dev);				// 如果设备已启用，则初始化该设备上的组播信息
...
static void inetdev_destroy(struct in_device *in_dev)
{
	...
	dev = in_dev->dev;
	if (dev == &loopback_dev)		// 不释放回环接口的IP配置块
		return;

	in_dev->dead = 1;				// 标识正在释放

	ip_mc_destroy_dev(in_dev);		// 清除组播配置
	// 删除释放所有的IP地址块
...
#ifdef CONFIG_SYSCTL
	devinet_sysctl_unregister(&in_dev->cnf);	// 注销IP相关的系统参数
#endif

	dev->ip_ptr = NULL;				// 清除net_device中的ip_ptr指针

#ifdef CONFIG_SYSCTL
	neigh_sysctl_unregister(in_dev->arp_parms);	// 注销邻居子系统中的协议参数配置
#endif
	neigh_parms_release(&arp_tbl, in_dev->arp_parms);	// 释放IP配置块中的邻居协议参数配置
	arp_ifdown(dev);			

	call_rcu(&in_dev->rcu_head, in_dev_rcu_put);  		// 通过RCU机制释放IP配置块
}
// 检查两个给定的IP地址是否属于一个子网
...
// dst: 0返回本地地址 !0返回与目的地址在同一个子网的IP地址        scope: RT_SCOPE_HOST RT_SCOPE_LINK RT_SCOPE_UNIVERSE
