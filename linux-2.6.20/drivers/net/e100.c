...
MODULE_DEVICE_TABLE(pci, e100_id_table);		// 导出支持的设备类型
...
static irqreturn_t e100_intr(int irq, void *dev_id)
{
	...
	if(stat_ack & stat_ack_rnr)
		nic->ru_running = RU_SUSPENDED;
	// 如果网络设备工作正常且系统未轮询接收报文，则禁止该网络设备的中断，并将该网络设备添加到网络设备轮询队列
	if(likely(netif_rx_schedule_prep(netdev))) {
		e100_disable_irq(nic);			// 禁止该设备硬件中断
		__netif_rx_schedule(netdev);	// 将该设备添加到轮询队列，更新网络设备读取报文数量的配额，激活接收报文软中断
	}
...
{
	struct nic *nic = netdev_priv(netdev);
	unsigned int work_to_do = min(netdev->quota, *budget);		// 读取的配额
	unsigned int work_done = 0;
	int tx_cleaned;
	// 调用这个函数从网络设备中读取接收到的报文，并由netif_receive_skb()输入到上层协议中
	e100_rx_clean(nic, &work_done, work_to_do);		// work_done为已读取的报文数
	tx_cleaned = e100_tx_clean(nic);				// 释放已发送出去的报文
	// 如果待输出和输入的报文都已处理完成，则退出轮询模式，并从网络设备轮询队列中删除该设备
...
static int e100_suspend(struct pci_dev *pdev, pm_message_t state)
{
	...
	if (netif_running(netdev))			// 如果网络设备处于激活状态，则等待网络设备完成轮询接收数据包
		netif_poll_disable(nic->netdev);
	del_timer_sync(&nic->watchdog);		// 删除监视网络设备工作状态的定时器
	netif_carrier_off(nic->netdev);		// 显示的调用这个函数，是设备驱动处于不可传递数据状态，并关闭网络设备的排队功能
